...
<form method="post" id="compose" name="message" action="?action=takeanswer">
    <?php if ($_GET["returnto"] || $_SERVER["HTTP_REFERER"]) { ?>
        <input type=hidden name=returnto
               value="<?php echo htmlspecialchars($_GET["returnto"]) ? htmlspecialchars($_GET["returnto"]) : htmlspecialchars($_SERVER["HTTP_REFERER"]) ?>">
    <?php } ?>
    ...
    if ($action == "takecontactanswered") {
        ...
        if ($_POST['setdealt']) {
            ...
            $res = sql_query("SELECT id FROM staffmessages WHERE answered=0 AND id IN (" . $setArr . ")");
            while ($arr = mysql_fetch_assoc($res))
            ...
            $res = sql_query("SELECT id FROM staffmessages WHERE id IN (" . $setArr . ")");
            while ($arr = mysql_fetch_assoc($res))
