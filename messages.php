...
<?php
print("<td width=\"1%\" class=\"colhead\" align=\"left\">$sender_receiver</td>");
?>
...
if ($row['unread'] == 'yes')
{
echo("<tr>\n<td class=rowfollow align=center><img class=\"bookmark\" src=\"pic/trans.gif\" alt=\"Unread\" title=".$lang_messages['title_unread']." /></td>\n");
}
else
{
echo("<tr>\n<td class=rowfollow align=center><img class=\"delbookmark\" src=\"pic/trans.gif\" alt=\"Read\" title=".$lang_messages['title_read']." /></td>\n");
}
...
?>
<tr class="colhead">
<td colspan="5" align="right" class="colhead"><input class=btn type="button" value="<?php echo $lang_messages['input_check_all']; ?>" onClick="this.value=check(form,'<?php echo $lang_messages['input_check_all'] ?>','<?php echo $lang_messages['input_uncheck_all'] ?>')"> <input class=btn type="button" value="反选" onClick="checktocheck(form)"> 
...
<div align="center"><img class="bookmark" src="pic/trans.gif" alt="Unread" title="<?php echo $lang_messages['title_unread'] ?>" /><a href="messages.php?action=viewmailbox&box=<?php echo $mailbox?>&unread=yes"><?php echo $lang_messages['text_unread_messages'] ?></a>
<img class="delbookmark" src="pic/trans.gif" alt="Read" title="<?php echo $lang_messages['title_read'] ?>" /><a href="messages.php?action=viewmailbox&box=<?php echo $mailbox?>&unread=no"><?php echo $lang_messages['text_read_messages'] ?></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
if($message['identify']){
sql_query("delete from  messages WHERE id=" . sqlesc($pm_id) . " AND receiver=" . sqlesc($CURUSER['id']));

if(preg_match("/@?topicid([0-9]+)page([0-9]+)/",$message['identify'], $matches))redirectmess("forums.php?action=viewtopic&topicid=".$matches[1]."&page=p".$matches[2]."#pid".$matches[2]);
elseif(preg_match("/@?torrent([0-9]+)/",$message['identify'], $matches))redirectmess('details.php?id='.$matches[1]);
elseif(preg_match("/@?offer([0-9]+)/",$message['identify'], $matches))redirectmess('offers.php?off_details=1&id='.$matches[1]);
elseif(preg_match("/@?request([0-9]+)/",$message['identify'], $matches))redirectmess('viewrequests.php?req_details=1&id='.$matches[1]);
}
...
//Mark a single message as read
	@sql_query("UPDATE messages SET unread='no' WHERE id=" . sqlesc($pm_id) . " AND receiver=" . $CURUSER['id']);
	}
	else
	{$pm_messages[]=0;
...
@sql_query("UPDATE messages SET location=" . sqlesc($pm_box) . " WHERE id=" . sqlesc($pm_id) . " AND receiver=" . $CURUSER['id']);
...
}elseif ($_POST['deletesystem']){
	sql_query("DELETE FROM messages WHERE sender=0 and location= ".PM_INBOX." and receiver=" . sqlesc($CURUSER['id']) ) or sqlerr(__FILE__,__LINE__);
	$Cache->delete_value('user_'.$CURUSER['id'].'_unread_message_count');
	$Cache->delete_value('user_'.$CURUSER['id'].'_inbox_count');
...
// Get the message
$res = sql_query('SELECT * FROM messages WHERE id=' . sqlesc($pm_id) . ' AND (receiver=' . sqlesc($CURUSER['id']) . ' OR sender=' . sqlesc($CURUSER['id']) .')') or sqlerr(__FILE__,__LINE__);
if (!$res)
{
...
<tr>
<td class="colhead" align=left>删除所有系统邮件(仅收件箱,包含未读)</td>
</tr>
<tr>
<td align=left>
<form action="messages.php" method="post">
<input type="hidden" name="action" value="moveordel"> 
<input type="submit" name="deletesystem" value="删除">
</form></td>
</tr>
<tr>
...
{
// Edit name
sql_query("UPDATE pmboxes SET name=" . sqlesc($_GET['edit' . $row['id']]) . " WHERE id=" . sqlesc($row['id']));
}
else
{
// Delete
sql_query("DELETE FROM pmboxes WHERE id=" . sqlesc($row['id']));
