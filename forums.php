...
<?php
	...
	$todaypostcount = $Cache->get_value('today_'.$today_date.'_posts_count');
	if ($todaypostcount=='') {
...
function insert_compose_frame($id, $type = 'new')
{
	...
	switch ($type){
		...
		case 'quote':
		{
			...
			$res = sql_query("SELECT posts.body, posts.userid , users.username FROM posts LEFT JOIN users ON posts.userid = users.id WHERE posts.id=$id") or sqlerr(__FILE__, __LINE__);
			if (mysql_num_rows($res) != 1)
				stderr($lang_forums['std_error'], $lang_forums['std_no_post_id']);
			$arr = mysql_fetch_assoc($res);
			$body = "[quote=".htmlspecialchars($arr["username"])."]".htmlspecialchars(unesc(format_quotesnopic($arr["body"])))."[/quote]";
			

			print("<input type=\"hidden\" name=\"quotepostuserid\" value=\"".$arr["userid"]."\" />");
...
if ($action == "post")
{
	...
	else
	{
		...
		if ($type == 'new')$timeout=60;
		else  $timeout=10;
		$addedtimeout=300;
		 
		if (strtotime($CURUSER['added']) > (TIMENOW - $addedtimeout)&&$type == 'new')
		{
				$secs = $addedtimeout - (TIMENOW - strtotime($CURUSER['added']));
				stderr($lang_forums['std_error'],$lang_forums['std_post_flooding2'].$secs.$lang_forums['std_seconds_before_making'],false);
		}
			
		
		
			if (strtotime($CURUSER['last_post']) > (TIMENOW - $timeout))
			{
				$secs = $timeout - (TIMENOW - strtotime($CURUSER['last_post']));
	...
	$arr["name"] = get_single_value("topics", "subject ", "WHERE id=".sqlesc($topicid));
	$athaveowner=at_user_message($body,("topicid".$topicid."page".$postid),$arr['name'],'topic',$arr["owner"]);
	...
	if ($type == "reply"){
	$qoute_cidid=0+$_POST["quotepostuserid"];
	
	$arr["owner"]   = get_single_value("topics", "userid", "WHERE id=".sqlesc($topicid));
	$arr["name"]   = get_single_value("topics", "subject ", "WHERE id=".sqlesc($topicid));
	...
	$arg = mysql_fetch_array($ras);
		...
		if($qoute_cidid){$rasqoute = sql_query("SELECT commentpm FROM users WHERE id = $qoute_cidid") or sqlerr(__FILE__,__LINE__);
		$argqoute = mysql_fetch_array($rasqoute);}
		
		//引用非自己
		$identify=sqlesc("topicid".$topicid."page".$postid);
		
		if($qoute_cidid&&$argqoute["commentpm"] == 'yes'&& $qoute_cidid != $CURUSER['id']){
			$added = sqlesc(date("Y-m-d H:i:s"));
			//$subject = sqlesc($lang_comment_target[get_user_lang($qoute_cidid)]['msg_new_comment2']."(". $arr['name'] . ")");
			$subject = sqlesc($lang_comment_target[get_user_lang($qoute_cidid)]['msg_torrent_receive_comment2']."(". $arr['name'] . ")");
			$notifs = sqlesc($lang_comment_target[get_user_lang($qoute_cidid)]['msg_torrent_receive_comment2'] . " [url=/forums.php?action=viewtopic&topicid=$topicid&page=p".$postid."#pid".$postid."] " . $arr['name'] . "[/url].");
			
			//if(get_row_count("messages","WHERE  msg  LIKE  '%forums%".$topicid."%' and sender = 0 and receiver=".$qoute_cidid ))
			sql_query("delete from messages where identify  LIKE  ".sqlesc("topicid".$topicid."%")." and sender = 0 and receiver=".$qoute_cidid);
			sql_query("INSERT INTO messages (sender, receiver, subject, msg, added,identify) VALUES(0, " . $qoute_cidid . ", $subject, $notifs, $added,$identify)") or sqlerr(__FILE__, __LINE__);
			$Cache->delete_value('user_'.$qoute_cidid.'_unread_message_count');
			$Cache->delete_value('user_'.$qoute_cidid.'_inbox_count');}
//}
		...
		//回复
		if($arg["commentpm"] == 'yes' && $CURUSER['id'] != $arr["owner"]&&!$athaveowner&&(!$qoute_cidid||$qoute_cidid&&$qoute_cidid!=$arr["owner"])){
			$added = sqlesc(date("Y-m-d H:i:s"));
			//$subject = sqlesc($lang_comment_target[get_user_lang($arr["owner"])]['msg_new_comment']."(". $arr['name'] . ")");
			$subject = sqlesc($lang_comment_target[get_user_lang($arr["owner"])]['msg_torrent_receive_comment']."(". $arr['name'] . ")");
			$notifs = sqlesc($lang_comment_target[get_user_lang($arr["owner"])]['msg_torrent_receive_comment'] . " [url=/forums.php?action=viewtopic&topicid=$topicid&page=p".$postid."#pid".$postid."] " . $arr['name'] . "[/url].");
		//if(get_row_count("messages","WHERE msg  LIKE  '%forums%".$topicid."%'   and sender = 0 and receiver=".$arr['owner'] ))
			sql_query("delete from messages where identify  LIKE  ".sqlesc("topicid".$topicid."%")." and sender = 0 and receiver=".$arr['owner']);
			sql_query("INSERT INTO messages (sender, receiver, subject, msg, added,identify) VALUES(0, " . $arr['owner'] . ", $subject, $notifs, $added,$identify)") or sqlerr(__FILE__, __LINE__);
...
if ($action == "viewtopic")
{
	...
	print("<h1 align=\"center\"><a class=\"faqlink\" href=\"forums.php\">".$SITENAME."&nbsp;".$lang_forums['text_forums']."</a>--><a class=\"faqlink\" href=\"".htmlspecialchars("?action=viewforum&forumid=".$forumid)."\">".$forumname."</a><b>--></b><span id=\"top\">".$subject.($locked ? "&nbsp;&nbsp;<b>[<font class=\"striking\">".$lang_forums['text_locked']."</font>]</b>" : "").get_topic_bookmark_state($CURUSER[id],$topicid)."</span></h1>\n");
	end_main_frame();
	print($pagertop);
	...
	begin_main_frame();
	print("<table border=\"0\" class=\"main\" cellspacing=\"0\" cellpadding=\"5\" width=\"98%\"><tr>\n");
	...
	if ($maypost)
	{
		;//print("<a href=\"".htmlspecialchars("?action=reply&topicid=".$topicid)."\" align=\"right\"><img class=\"f_reply\" src=\"pic/trans.gif\" alt=\"Add Reply\" title=\"".$lang_forums['title_reply_directly']."\" /></a>&nbsp;&nbsp;");
	}
	print("</td>");
	print("</tr></table>\n");
	begin_frame();
	...
	while ($arr = mysql_fetch_assoc($res))
	{
		...
		if (is_valid_id($arr['editedby']))
			print("|&nbsp;&nbsp;[<a href=\"comment.php?action=vieworiginal&amp;cid=".$arr[id]."&amp;type=forums\">".$lang_functions['text_view_original']."</a>]");
		...
		if (is_valid_id($arr['editedby']))
		{
			...
			$body .= "<br /><br /><br /><p><font class=\"small\">".$lang_forums['text_last_edited_by'].get_username($arr['editedby']).$lang_forums['text_last_edit_at'].$lastedittime."</font></p>\n";
		}
		$body .= "</div>";
		if ($signature)
		...
		$stats = "<br />
		"."&nbsp;&nbsp;".$lang_forums['text_posts']."$forumposts<br />
		"."&nbsp;&nbsp;".$lang_forums['text_ul']."$uploaded <br />
		"."&nbsp;&nbsp;".$lang_forums['text_dl']."$downloaded<br />
		"."&nbsp;&nbsp;".$lang_forums['text_ratio']."$ratio<br />".
		($showschool == 'yes'?"&nbsp;&nbsp;院校： ".return_school_name($arr2['school'])."<br />":"<br />");
		print("<tr><td rowspan=\"2\" class=\"rowfollow\" width=\"150\" valign=\"top\" align=\"left\" style='padding: 0px'>" .
		return_avatar_image($avatar). "<br />&nbsp;&nbsp;<img alt=\"".get_user_class_name($arr2["class"],false,false,true)."\" title=\"".get_user_class_name($arr2["class"],false,false,true)."\" src=\"".$uclass."\" />".$stats."</td>
		<td class=\"rowfollow\" valign=\"middle\" style='border-bottom: 0px;word-break:break-all;'><br />".$body."</td></tr>\n");
		$secs = 900;
		...
		if ($maypost){
		print("<a href=\"#qrbody\"  onclick=\"javascript:quick_reply_to('".($pn+$offset)."的[@".$arr2["id"]."]');\"><img class=\"f_reply\" src=\"pic/trans.gif\" alt=\"Add Reply\" title=\"".$lang_functions['title_add_reply']."\" /></a>");
...
<option style='background-color: white' value=\"40\">White</option>");
		...
		print("</tr>\n");
		print("</table>\n");
	}elseif($CURUSER["id"]==$forumtopicsowner&&!$locked){
		print("</td></tr><tr><td class=\"toolbox\" align=\"center\">\n");
		print("<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" align=\"center\">\n");
		...
		print("<input type=\"hidden\" name=\"topicid\" value=\"".$topicid."\" />\n");
		print("<input type=\"hidden\" name=\"returnto\" value=\"".htmlspecialchars($_SERVER[REQUEST_URI])."\" />\n");
		print("<input type=\"hidden\" name=\"locked\" value=\"yes\" /><input type=\"submit\" class=\"medium\" value=\"".$lang_forums['submit_lock']."\" onclick='return confirm(\"确认锁定该主题吗?\");'/></form></td>\n");
		print("</tr>\n");
		print("</table>\n");
	}
...
if ($action == "setlocked")
{
	...
	;
	}elseif($CURUSER["id"]==get_single_value('topics', 'userid', "where id= $topicid")){
	$_POST["locked"]='yes';
	}else
		permissiondenied();
...
if ($action == "viewforum")
{
	...
	list($pagertop, $pagerbottom, $limit) = pager($topicsperpage, $num, "?"."action=viewforum&forumid=".$forumid.$addparam."&".($_GET["sort"]?"sort=".$_GET["sort"]."&":""));
	if ($_GET["sort"]){
		switch ($_GET["sort"]){
			case 'firstpostasc': 
			{
			...
			case 'postcolour':
			{
				$orderby = "hlcolor > 0 DESC,lastpost DESC";
				break;
			}
	...
	print("</td><td class=\"embedded nowrap\" width=\"1%\" align=\"right\">");
	if ($maypost)
	...
	if ($numtopics > 0)
	{
		...
		print("<tr>
		<tr><td class=\"colhead\" align=\"center\" width=\"99%\"><a href=\"".htmlspecialchars("?action=viewforum&forumid=".$forumid.$addparam."&sort=".($_GET["sort"] == 'postcolour' ? "none" : "postcolour"))."\" title=\"".($_GET["sort"] == 'postcolour' ?  "默认" : "按颜色降序")."\">".$lang_forums['col_topic']."</a></td>
		...
		while ($topicarr = mysql_fetch_assoc($topicsres))
		{
			...
			$fpuserid = $topicarr['userid'];//;0 + $arr["userid"];
		...
		//print("</table>\n");
		//print("<table border=\"0\" cellspacing=\"0\" cellpadding=\"5\" width=\"98%\">");
		print("<tr><td align=\"left\" class=\"colhead\">\n");
		...
		print("</td>");
?>
<td align="left" colspan="3" class="colhead">
<span id="orderlist" class="dropmenu" style="display: none;margin-top: -155px;"><ul>
...
<li><a href="?action=viewforum&amp;forumid=<?php echo $forumid.$addparam?>&amp;sort=postcolour">主题颜色降序</a></li>

</ul>
</span>
<span id="order" onclick="dropmenu(this);"><span style="cursor: pointer;"><b>点击选择<?php echo $lang_forums['text_order']?></b></span><br/>
...
if ($action == "search")
{
	...
	if ($keywords != "")
	{
		...
		$res = sql_query("SELECT COUNT(posts.id) FROM posts LEFT JOIN topics ON posts.topicid = topics.id LEFT JOIN forums ON topics.forumid = forums.id WHERE forums.minclassread <= ".sqlesc(get_user_class())." AND (topics.subject $extraSql AND posts.id=topics.firstpost) ") or sqlerr(__FILE__, __LINE__);
		$arr = mysql_fetch_row($res);
		$hits = 0 + $arr[0];
		if ($hits){
			$error = false;
...
<?php
	...
	if (!$error)
	{
		...
		list($pagertop, $pagerbottom, $limit) = pager($perpage, $hits, "forums.php?action=search&keywords=".rawurlencode($keywords)."&".($_GET["sort"]?"sort=".$_GET["sort"]."&":""));
...
if ($action == "bookmark")
{
	...
	$num = get_row_count("topics","left join (SELECT topicid,userid,time_book FROM bookmarks_topic WHERE userid=" . $CURUSER['id'] . ") as bookmarks_topic  on topicid=topics.id where  bookmarks_topic.userid=".sqlesc($userid).$wherea);
	...
	{
		$orderby = "time_book DESC";
	}
	//------ Get topics data
	$topicsres = sql_query("SELECT * FROM topics left join (SELECT topicid,userid,time_book FROM bookmarks_topic WHERE userid=" . $CURUSER['id'] . ") as bookmarks_topic  on topicid=topics.id where  bookmarks_topic.userid=".sqlesc($userid).$wherea." ORDER BY sticky DESC,".$orderby." ".$limit) or sqlerr(__FILE__, __LINE__);
	$numtopics = mysql_num_rows($topicsres);
	stdhead($lang_forums['head_forum']." ".$forumname);
	begin_main_frame("",true);
	print("<h1 align=\"center\"><a class=\"faqlink\" href=\"forums.php\">".$SITENAME."&nbsp;".$lang_forums['text_forums'] ."</a>--><a class=\"faqlink\" href=\"".htmlspecialchars("forums.php?action=bookmark")."\">".$forumname."</a></h1>\n");
	end_main_frame();
	print("<br />");
	...
	if ($numtopics > 0)
	{
		...
		print("<tr><td class=\"colhead\" align=\"center\" width=\"99%\"><a href=\"".htmlspecialchars("?action=bookmark".$addparam."&sort=".($_GET["sort"] == 'postcolour' ? "none" : "postcolour"))."\" title=\"".($_GET["sort"] == 'postcolour' ?  "默认" : "按颜色降序")."\">".$lang_forums['col_topic']."</a></td>
		<td class=\"colhead\" align=\"center\"><a href=\"".htmlspecialchars("?action=bookmark".$addparam."&sort=".($_GET["sort"] == 'firstpostdesc' ? "firstpostasc" : "firstpostdesc"))."\" title=\"".($_GET["sort"] == 'firstpostdesc' ?  $lang_forums['title_order_topic_asc'] : $lang_forums['title_order_topic_desc'])."\">".$lang_forums['col_author']."</a></td>
		
		<td class=\"colhead\" align=\"center\">".$lang_forums['col_replies']."/".$lang_forums['col_views']."</td>
		
		<td class=\"colhead\" align=\"center\"><a href=\"".htmlspecialchars("?action=bookmark".$addparam."&sort=".($_GET["sort"] == 'lastpostasc' ? "lastpostdesc" : "lastpostasc"))."\" title=\"".($_GET["sort"] == 'lastpostasc' ? $lang_forums['title_order_post_desc'] : $lang_forums['title_order_post_asc'])."\">".$lang_forums['col_last_post']."</a></td>\n");
		...
		while ($topicarr = mysql_fetch_assoc($topicsres))
		{
			...
			$subject = ($sticky ? "<img class=\"sticky\" src=\"pic/trans.gif\" alt=\"Sticky\" title=\"".$lang_forums['title_sticky']."\" />&nbsp;&nbsp;" : "") . "<a href=\"".htmlspecialchars("?action=viewtopic&topicid=".$topicid)."\" ".$onmouseover.">" .highlight_topic(highlight($search,htmlspecialchars($topicarr["subject"])), $hlcolor) . "</a>".$topicpages.get_topic_bookmark_state($CURUSER[id],$topicid,$counter);
			$lastpostread = get_last_read_post_id($topicid);
		...
		print("<tr><td align=\"left\" class=\"colhead\">\n");
		print("<form method=\"get\" action=\"forums.php\"><b>".$lang_forums['text_fast_search']."</b><input type=\"hidden\" name=\"action\" value=\"bookmark\" /><input type=\"hidden\" name=\"forumid\" value=\"".$forumid."\" /><input type=\"text\" style=\"width: 180px\" name=\"search\" />&nbsp;<input type=\"submit\" value=\"".$lang_forums['text_go']."\" /></form>");
		print("</td>");
?>
<td align="left" colspan="3" class="colhead">
<span id="orderlist" class="dropmenu" style="display: none;margin-top: -155px;"><ul>
<li><a href="?action=bookmark&amp;sort=firstpostdesc"><?php echo $lang_forums['text_topic_desc']?></a></li>
<li><a href="?action=bookmark&amp;sort=firstpostasc"><?php echo $lang_forums['text_topic_asc']?></a></li>
<li><a href="?action=bookmark&amp;sort=lastpostdesc"><?php echo $lang_forums['text_post_desc']?></a></li>
<li><a href="?action=bookmark&amp;sort=lastpostasc"><?php echo $lang_forums['text_post_asc']?></a></li>
<li><a href="?action=bookmark&amp;sort=postcolour">主题颜色降序</a></li>
...
print("<p align=\"center\"><a href=\"?action=search\"><b>".$lang_forums['text_search']."</b></a> | <a href=\"?action=viewunread\"><b>".$lang_forums['text_view_unread']."</b></a> | <a href=\"?catchup=1\"><b>".$lang_forums['text_catch_up']."</b></a> | <a href=\"?action=bookmark\"><b>查看收藏</b></a> ".(get_user_class() >= $forummanage_class ? "| <a href=\"forummanage.php\"><b>".$lang_forums['text_forum_manager']."</b></a>":"")."</p>");
...
foreach ($overforums as $a)
{
	...
	foreach ($forums as $forums_arr)
	{
		...
		$arr = $Cache->get_value('forum_'.$forumid.'_last_replied_topic_content');
		if ($arr==''){
