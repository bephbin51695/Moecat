<?php
 ...
 function countobject($object,$name){
...
function settypt ($id) {
 $this->imdbtype = $id;
  ...
  function cachestate ($target = "",$checkphoto=true) {
   ...
   if ($this->usecache)
   {
	...
	foreach($ext_arr as $ext)
  	{
	   	if(!file_exists($this->cachedir."/".$this->imdbID.".".strtolower($ext)))
			return 0;
	    @$fp = fopen ($this->cachedir."/".$this->imdbID.".".strtolower($ext), "r");
	    if (!$fp)
	    	return 0;
  	}
  ...
  // case "Taglines": $urlname=""; break;
   ...
   if ($this->usecache) {
    @$fp = fopen ("$this->cachedir/$this->imdbID.".strtolower($wt), "r");
    if ($fp) {
     $temp="";
     while (!feof ($fp)) {
	$temp .= fread ($fp, 1024);
...
if($this->imdbtype==2)
$url="https://api.douban.com/v2/movie/".$this->imdbID."?apikey=".doubanapikey();
else
$url="https://api.douban.com/v2/movie/imdb/tt".$this->imdbID."?apikey=".doubanapikey();
$this->page[$wt] = file_get_contents_function($url,30);


   if($this->page[$wt] ){ //storecache
    if ($this->storecache) {
     $fp = fopen ("$this->cachedir/$this->imdbID.".strtolower($wt), "w");
  ...
  function purge_single($explicit = false,$explicitmust = false,$days=12) {
    if (is_dir($this->cachedir)) 
    {
      ...
      {
	      $fname = $this->cachedir ."/". $this->imdbid() . "." . strtolower($ext);
...
function purge_single_jpg($delete=false) {
  ...
  ($this->main_title = $this->page["Title"]->title);
  }
  return $this->main_title;
   ...
   if ($this->main_episodes == "") {
	...
	$this->main_episodes = countarray($this->page["Title"]->attrs->episodes);
...
$this->main_year=countarray($this->page["Title"]->attrs->pubdate);
  ...
  function runtime_all () {
   if ($this->main_runtime == "") {
	...
	$this->main_runtime = countarray($this->page["Title"]->attrs->movie_duration);
    }
...
#echo $this->runtime_all();
  ...
  function rating () {
   if ($this->main_rating == "")
   {
	...
	$this->main_rating=$this->page["Title"]->rating->average;
...
$this->main_votes =$this->page["Title"]->rating->numRaters;
...
$this->main_language = countarray($this->page["Title"]->attrs->language);
   }
  ...
  function genres () {
   if ($this->main_genres == "") {
    if ($this->page["Title"] == "") $this->openpage ("Title");
	 ...
	 $this->main_genres = countarray($this->page["Title"]->attrs->movie_type);
  ...
  function tagline () {
   if ($this->main_tagline == "") {
    if ($this->page["Title"] == "") $this->openpage ("Title");
	$this->main_tagline = countobject($this->page["Title"]->tags,name);
	}
  ...
  function plot () {
   if ($this->plot_plot == "") {
    if ($this->page["Title"] == "") $this->openpage ("Title");
	$this->plot_plot = nl2br($this->page["Title"]->summary);
   }
...
$this->credits_director = countarray($this->page["Title"]->attrs->director);
...
preg_match_all( "/(https:\/\/movie\.douban\.com\/movie\/[0-9]*)/", $this->page["Title"]->alt, $zh_CN);
...
return $this->doubantureid;
  ...
  function creator(){
   if ($this->main_creator == "") {
	...
	$this->main_creator=countobject($this->page["Title"]->author,name);
  ...
  function cast () {
	...
	$this->credits_cast = countarray($this->page["Title"]->attrs->cast);
	}
   //}
  ...
  function writing () {
   if ($this->credits_writing == "") {
	...
	$this->credits_writing= countarray($this->page["Title"]->attrs->writer);
	}
  ...
  function producer () {
   if ($this->credits_producer == "") {
	...
	//$this->credits_producer= countarray($this->page["Title"]->attrs->writer);
  ...
  function link () {
   if ($this->main_link == "")
   {
	...
	$this->main_link  = str_replace("movie/","subject/",$this->page["Title"]->alt);
  ...
  function photo () {
   if ($this->main_photo == "")
   {
    if ($this->page["Title"] == "") $this->openpage ("Title");
		...
		$this->main_photo = $this->page["Title"]->image;
	
	
	$doubandoubanimg_httpextension = array('img0.douban.com','img1.douban.com','img2.douban.com','img3.douban.com','img4.douban.com','img5.douban.com');
	$doubandoubanimg_httpextension=$doubandoubanimg_httpextension[mt_rand (0,count($doubandoubanimg_httpextension)-1)];

	$this->main_photo= str_replace("spic","lpic",$this->main_photo);
	$this->main_photo= str_replace("photo/icon","photo/photo",$this->main_photo);
	$this->main_photo= str_replace("movie_poster_cover/ipst","photo/photo",$this->main_photo);
	//$this->main_photo= preg_replace("/img[0-9]{1}\.douban\.com/is",$doubandoubanimg_httpextension,$this->main_photo);
	//die($this->main_photo);
	if (preg_match("/default/i", $URL))$this->main_photo='';
   }
  ...
  function savephoto ($path) {  
   ...
   if (strpos($response->getHeader("Content-Type"),'image/jpeg') === 0
	...
	if(!$fp)$fp=file_get_contents_function($photo_url,40);
   $fp2 = fopen ($path, "w");
   if(!$fp2)$fp2 = fopen ($path, "w");
   
   if ((!$fp) || (!$fp2)){
     echo "image error...".$photo_url."<BR>";
     return false;
   }
   ...
   fclose ($fp2);
	...
	if(!$orig=@imagecreatefromjpeg($path))return write_log("AUTO_IMDBUPDATE_ERROR_IMG".$path,'file');
	imageinterlace($orig ,true);
	imagejpeg($orig, $path,70);
	return true;
}
  ...
  function country () 
  {
   if ($this->main_country == "") 
   {
	...
	$this->main_country=countarray($this->page["Title"]->attrs->country);
 ...
 function alsoknow  () {
   if ($this->alsoknow == "") {
	...
	$this->alsoknow=($this->page["Title"]->alt_title);
}
...
$this->alsoknowcnname=$this->page["Title"]->title;
   }
   return $this->alsoknowcnname;
  }
 
function movieallinfo  (){
return $this->rating ()." imdb".$this->imdbtureid() .
" douban".$this->doubantureid()." ".$this->title ().
$this->alsoknow().$this->country ().$this->genres().
$this->tagline().
$this->director().
$this->creator().
$this->writing().$this->producer().$this->cast();
}
