...
if (isset($_GET['del'])) {
    if (is_valid_id($_GET['del'])) {
        if ((get_user_class() >= $sbmanage_class)) {    //checkloggedinorreturn();
...
<head>
    ...
    <style type="text/css">
        ...
        .shoutrowbak {
            font-family: Lucida Console, Microsoft YaHe, tahoma, arial, helvetica, sans-serif;
        }
    </style>
    <?php
    ...
    <script type="text/javascript">
        ...
        function startcountdown(time) {
            ...
            setTimeout("startcountdown(" + time + ")", 1000);
        }
...
if ($_POST["type"] != "helpbox") {
    ...
    ?> onload="<?php echo $startcountdown ?>;hbquota()" <?php } ?>>
<?php
if ($_POST["sent"] == "yes") {
    if (!$_POST["shbox_text"]) {
        $userid = 0 + $CURUSER["id"];
