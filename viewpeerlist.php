...
function dltable($name, $arr, $torrent,$count,$type)
{
	...
	(1||$enablelocation_tweak == 'yes' || get_user_class() >= $userprofile_class ? "<td class=colhead align=center width=1% >".$lang_viewpeerlist['col_location']."</td><td class=colhead align=center width=1% >".$lang_viewpeerlist['col_location_type']."</td>" : "").
	...
	foreach ($arr as $e) {
		...
		elseif (get_user_class() >= $userprofile_class){
		...
		else {$s .= "<td class=rowfollow align=center width=1%><nobr>未知</nobr></td>\n";}
			...
			$iptypesecond="";
			$s .= "<td class=rowfollow align=left width=1%><nobr>" . $iptype.$xinhao.$iptypesecond. "</nobr></td>\n";
